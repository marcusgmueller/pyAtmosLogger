...
class ott_parsivel2_default:
    ...
    def convert(self, file):
        ...
        for iKey in range(2):
            ...
            newDF = pd.DataFrame(list(zip(datetimeList, classList, valueList)), columns =['datetime', classKeys[iKey], keys[iKey]])
            newDF.set_index(['datetime', classKeys[iKey]], inplace=True)
        ...
        newDF = pd.DataFrame(list(zip(datetimeList, vedClassList, rofClassList, MList)), columns =['datetime', 'ved_class', 'rof_class', "M"])

        newDF.set_index(["datetime", "ved_class","rof_class"], inplace=True)
        ds = ds.merge(newDF.to_xarray())
        #set attributes
        ...
        ds.N.attrs                  = {'units': 'log_10(1/m^3 mm)', 'description': 'average volume equivalent diameter (ved)'}
        ds.v.attrs                  = {'units': 'm/s', 'description': 'average rate of fall (rof)'}
        ds.M.attrs                  = {'units': '1', 'description': 'raw data matrix. number of particles per volume diameter and fall velocity'}
        ds.ved_class.attrs          = {'units': '', 'description': 'volume equivalent diameter (ved) class'}
        ds.rof_class.attrs          = {'units': '', 'description': 'average rate of fall (rof) class'}
